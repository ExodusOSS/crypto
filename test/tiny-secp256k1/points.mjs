...
const fpoints = loadFixture('points')

export default function(secp256k1, type) {
  test(`isPoint (${type})`, (t) => {
    for (const f of fpoints.valid.isPoint) {
      const p = fromHex(f.P)
      t.equal(secp256k1.isPoint(p), f.expected, `${f.P} is ${f.expected ? 'OK' : 'rejected'}`)
    }
  ...
  test(`isPointCompressed (${type})`, (t) => {
    for (const f of fpoints.valid.isPoint) {
      ...
      const p = fromHex(f.P)
      const e = p.length === 33
      t.equal(secp256k1.isPointCompressed(p), e, `${f.P} is ${e ? 'compressed' : 'uncompressed'}`)
    }
  ...
  test(`isXOnlyPoint (${type})`, (t) => {
    for (const f of fpoints.valid.isPoint) {
      ...
      t.equal(secp256k1.isXOnlyPoint(p2), e, `${f.P} is ${e ? 'xonly' : 'uncompressed'}`)
    }
  ...
  test(`pointAdd (${type})`, (t) => {
    for (const f of fpoints.valid.pointAdd) {
      ...
      t.same(secp256k1.pointAdd(p, q, true), secp256k1.pointCompress(expected, true), description)
      t.same(secp256k1.pointAdd(p, q, false), secp256k1.pointCompress(expected, false), description)
    }
